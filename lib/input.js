...
function _createInputDescriptor (input, inputOptions, containerOptions) {
  ...
  if (is.object(inputOptions)) {
    ...
    if (is.defined(inputOptions.text)) {
      if (is.object(inputOptions.text) && is.string(inputOptions.text.value)) {
        ...
        inputDescriptor.textValue = inputOptions.text.value;
        inputDescriptor.textWidth = (is.integer(inputOptions.text.width) && is.inRange(inputOptions.text.width, 1, this.constructor.maximum.width) && inputOptions.text.width) || 0;
        inputDescriptor.textHeight = (is.integer(inputOptions.text.height) && is.inRange(inputOptions.text.height, 1, this.constructor.maximum.height) && inputOptions.text.height) || 0;
        inputDescriptor.textAlign = (is.string(inputOptions.text.align) && is.inArray(inputOptions.text.align, Object.keys(alignReplacements)) && alignReplacements[inputOptions.text.align]) || 'low';
        const textColor = color(inputOptions.text.color || inputOptions.text.colour);
        inputDescriptor.textColor = [
          textColor.red(),
          textColor.green(),
          textColor.blue(),
          Math.round(textColor.alpha() * 255)
        ];
        const backgroundColor = color(inputOptions.text.backgroundColor || inputOptions.text.backgroundColour || '#0000');
        inputDescriptor.textBackground = [
          backgroundColor.red(),
          backgroundColor.green(),
          backgroundColor.blue(),
          Math.round(backgroundColor.alpha() * 255)
        ];
        inputDescriptor.textFont = (is.string(inputOptions.text.font) && inputOptions.text.font) || 'sans';
        inputDescriptor.textFontSize = (is.integer(inputOptions.text.fontSize) && inputOptions.text.fontSize) || null;
        inputDescriptor.textGravity = (is.string(inputOptions.text.gravity) && is.inArray(inputOptions.text.gravity, ['north-west', 'north', 'north-east', 'west', 'center', 'east', 'south-west', 'south', 'south-east']) && inputOptions.text.gravity) || 'center';
        inputDescriptor.textLinespacing = (is.number(inputOptions.text.lineSpacing) && inputOptions.text.lineSpacing) || 0;
        delete inputDescriptor.buffer;
      } else {
        throw new Error('Expected a text value to create a new text image');
      }
    }
